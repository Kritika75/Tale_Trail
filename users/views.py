...
def login(request):
    callback_url = request.build_absolute_uri(reverse("users:callback"))
    print("Callback URL:", callback_url)  # Log the callback URL
...
def logout(request):
    ...
    return redirect(
        f"https://{settings.AUTH0_DOMAIN}/v2/logout?" + urlencode({
            "returnTo": request.build_absolute_uri('/'),  # Set this to the desired landing page after logout
            "client_id": settings.AUTH0_CLIENT_ID,
